...
const Header = () => {
  ...
  return (
    <div className="bg-header-color text-white fixed z-10 w-full top-0 left-0">
      ...
      >
        <Link to="/" onClick={logoFunc}>
          <img src={logo} alt="" className="w-28 h-22"></img>
        </Link>
        ...
        <div className="space-x-0 md:space-x-4 lg:space-x-8 flex items-center">
          ...
          <div className="relative cursor-pointer" onClick={flagFunc}>
            <TfiWorld size={28}/>
            {flagshow ? <FlagDropDown></FlagDropDown> : ""}
          </div>

          <div className="space-x-4 lg:space-x-8 hidden md:flex items-center">
            <Link to="/" onClick={logoFunc}>{t('mainpage.home')}</Link>
            <Link to="/menu" onClick={logoFunc}>{t('mainpage.menu')}</Link>
            <a href="#about" onClick={logoFunc}>{t('mainpage.about-header')}</a>
            <a href="#contact" onClick={logoFunc}>{t('mainpage.contact')}</a>
          </div>
        </div>
